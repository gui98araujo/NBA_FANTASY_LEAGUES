...
# === Additional Code for Player Insights ===

# Add columns for games with and without teammate
results.append({
    "Teammate": tm_name,
    "With": avg_with,
    "Without": avg_without,
...
# Add KPIs for FPTS contribution ranks
stat_contrib = pd.DataFrame({
    "PTS": s["points"] * p_games["PTS"],
    "AST": s["assist"] * p_games["AST"],
    "STL": s["steal"] * p_games["STL"],
    "BLK": s["block"] * p_games["BLK"],
    "OREB": s["oreb"] * p_games["OREB"],
    "DREB": s["dreb"] * p_games["DREB"],
    "TOV": s["turnover"] * p_games["TOV"],
    "FOUL": s["tech_foul"] * p_games.get("TECH", 0) + s["flagrant_foul"] * p_games.get("FLAGRANT", 0)
})

avg_stat_contrib = stat_contrib.mean().to_dict()

# Compute ranks for each stat
stat_ranks = {}
for stat in avg_stat_contrib:
    df_season["stat_fp"] = s.get(stat.lower(), 0) * df_season.get(stat, 0)
    per_player_stat = df_season.groupby("PLAYER_ID")["stat_fp"].mean().sort_values(ascending=False)
    rank = per_player_stat.index.get_loc(sel_player_id) + 1 if sel_player_id in per_player_stat.index else None
    stat_ranks[stat] = rank

# Display additional KPIs
k5, k6, k7, k8, k9, k10, k11, k12 = st.columns(8)
with k5:
    st.metric("Rank by PTS FPTS", f"#{stat_ranks.get('PTS', 'N/A')}")
with k6:
    st.metric("Rank by OREB FPTS", f"#{stat_ranks.get('OREB', 'N/A')}")
with k7:
    st.metric("Rank by DREB FPTS", f"#{stat_ranks.get('DREB', 'N/A')}")
with k8:
    st.metric("Rank by BLK FPTS", f"#{stat_ranks.get('BLK', 'N/A')}")
with k9:
    st.metric("Rank by STL FPTS", f"#{stat_ranks.get('STL', 'N/A')}")
with k10:
    st.metric("Rank by AST FPTS", f"#{stat_ranks.get('AST', 'N/A')}")
with k11:
    st.metric("Avg FPTS lost by TO", f"{-avg_stat_contrib.get('TOV', 0):.2f}")
with k12:
    st.metric("Avg FPTS lost by FOUL", f"{-avg_stat_contrib.get('FOUL', 0):.2f}")


# === Additional Code for League Insights ===

# League-wide team defense insights
team_defense = df_s.groupby("OPPONENT_ABBREVIATION").agg({
    "PTS": lambda x: s["points"] * x.mean(),
    "OREB": lambda x: s["oreb"] * x.mean(),
    "DREB": lambda x: s["dreb"] * x.mean(),
...
# Display tables
st.markdown("### FPTS allowed per stat by team")
for stat in ["PTS", "OREB", "DREB", "AST", "STL", "BLK", "TOV"]:
    st.markdown(f"#### {stat}")
    stat_df = team_defense[["Team", stat]].sort_values(stat, ascending=False)
